...
app.get('/api/posts', (req, res) => {
    const rows = db.prepare(`
      SELECT p.id, p.title, p.description, p.image_path, p.created_at, p.product_link,
             u.id as user_id, COALESCE(u.name, u.email) as user_name, u.profile_image,
             (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as like_count,
             (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comment_count,
             p.audio_data IS NOT NULL as has_audio
...
app.get('/api/posts/:id', (req, res) => {
    ...
    const row = db.prepare(`
      SELECT p.id, p.title, p.description, p.story, p.image_path, p.created_at, p.product_link,
             u.id as user_id, COALESCE(u.name, u.email) as user_name, u.profile_image,
             (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as like_count,
             (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comment_count,
             p.audio_data IS NOT NULL as has_audio
...
app.post('/api/posts', upload.single('image'), async (req, res) => {
    if (!req.user) return res.status(401).json({ error: 'unauthorized' });
    const { title, description, storyType, productLink } = req.body || {};
    ...
    try {
        ...
        python.on('close', (code) => {
            ...
            console.log(`Story type: ${effectiveStoryType}`);
            ...
            if (code === 0) {
                try {
                    ...
                    if (result.success) {
                        ...
                        if (result.audio_path) {
                            try {
                                audioData = fs.readFileSync(result.audio_path);
                        ...
                        const stmt = db.prepare(`INSERT INTO posts (user_id, story, image_path, audio_data, product_link) VALUES (?, ?, ?, ?, ?)`);
                        const info = stmt.run(req.user.id, result.story, imagePath, audioData, productLink || null);
                        res.json({ id: info.lastInsertRowid, image_path: imagePath, has_audio: !!audioData });
                    } else {
                        console.error('Python script error:', result.error);
                        // Fallback to enhanced story
                        const generatedStory = createFallbackStory(description, effectiveStoryType);
                        const stmt = db.prepare(`INSERT INTO posts (user_id, story, image_path, audio_data, product_link) VALUES (?, ?, ?, ?, ?)`);
                        const info = stmt.run(req.user.id, generatedStory, imagePath, null, productLink || null);
                ...
                // Fallback to enhanced story
                const generatedStory = createFallbackStory(description, effectiveStoryType);
                const stmt = db.prepare(`INSERT INTO posts (user_id, story, image_path, audio_data) VALUES (?, ?, ?, ?)`);
...
addColumnIfNotExists('posts', 'audio_data', 'BLOB');
addColumnIfNotExists('posts', 'product_link', 'TEXT');
db.exec(`CREATE TABLE IF NOT EXISTS posts (
  ...
  story TEXT,
  image_path TEXT,
  audio_data BLOB, -- changed from audio_path
...
app.get('/api/posts/:id/audio', (req, res) => {
    const postId = Number(req.params.id);
    const row = db.prepare('SELECT audio_data FROM posts WHERE id = ?').get(postId);
    if (!row || !row.audio_data) {
        return res.status(404).json({ error: 'Audio not found' });
    }
    res.set('Content-Type', 'audio/mpeg'); // or appropriate mime type
